  ...
  try {
    ...
    const trialBalance = ledgers.map(ledger => {
      ...
      const debits = journalEntries
        .filter(entry => entry.debitLedgerId === ledger.id)
        .reduce((sum, entry) => sum + entry.amount, 0);
      
      // Sum credits where this ledger is credited
      const credits = journalEntries
        .filter(entry => entry.creditLedgerId === ledger.id)
        .reduce((sum, entry) => sum + entry.amount, 0);
      ...
      let closingType = closingBalance >= 0 ? 'DEBIT' : 'CREDIT';
...
exports.getProfitLoss = async (req, res) => {
  try {
    ...
    const calculateLedgerBalance = (ledgerId) => {
      const debits = journalEntries
        .filter(entry => entry.debitLedgerId === ledgerId)
        .reduce((sum, entry) => sum + entry.amount, 0);
      
      const credits = journalEntries
        .filter(entry => entry.creditLedgerId === ledgerId)
        .reduce((sum, entry) => sum + entry.amount, 0);

      return credits - debits; // Net credit is income/revenue
    };

    // Income ledgers
    const incomeLedgers = ledgers.filter(l => 
      ['INCOME', 'SALES_ACCOUNTS', 'DIRECT_INCOME', 'INDIRECT_INCOME'].includes(l.ledgerGroup)
    ).map(ledger => ({
      ledgerName: ledger.ledgerName,
      ledgerGroup: ledger.ledgerGroup,
    ...
    const expenseLedgers = ledgers.filter(l => 
      ['EXPENSES', 'PURCHASE_ACCOUNTS', 'DIRECT_EXPENSES', 'INDIRECT_EXPENSES'].includes(l.ledgerGroup)
    ).map(ledger => ({
      ledgerName: ledger.ledgerName,
      ledgerGroup: ledger.ledgerGroup,
  ...
  try {
    ...
    const calculateLedgerBalance = (ledger) => {
      ...
      return {
        ...
        type: balance >= 0 ? ledger.balanceType : (ledger.balanceType === 'DEBIT' ? 'CREDIT' : 'DEBIT'),
      };
    };
